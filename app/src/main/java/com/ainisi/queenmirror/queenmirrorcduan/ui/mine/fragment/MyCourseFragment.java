...
import com.ainisi.queenmirror.queenmirrorcduan.ui.mine.adapter.MyCourseListAdapter;
 ...
 *
 * @author jiangchao
 *         created at 2018/9/10 下午4:57
 */
public class MyCourseFragment extends Fragment implements HttpCallBack {
    ...
    List<MyCourseListBean.BodyEntity.ListEntity> listEntityList;
    private String tabName;

    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View contentView = inflater.inflate(R.layout.fragment_my_course, null);
//        ((TextView) contentView.findViewById(R.id.tv_content)).setText(getArguments().getString(EXTRA_CONTENT));
    ...
    private void getData() {
        ...
        HttpUtils.doPost(ACTION.MY_CLASS, map, CacheMode.REQUEST_FAILED_READ_CACHE, true, this);
    }
    ...
    public void onSuccess(int action, String res) throws WriterException {
        switch (action) {
            case ACTION.MY_CLASS:
                MyCourseListBean myCourseListBean = GsonUtils.jsonToBean(res, MyCourseListBean.class);
                if (myCourseListBean.isSuccess()) {
                    if ("6".equals(myCourseListBean.getErrorCode())) {
...
//                        totalPage = (myCourseListBean.getBody().getPageSum() / pageSize);
                        //得到数据
                        listEntityList = myCourseListBean.getBody().getList();
    ...
    MyCourseListBean.BodyEntity.ListEntity myCourseListEntity;
    ...
    private void showData() {
        ...
        switch (curState) {
            case STATE_INIT:
                //初始化状态，初始化列表
                mAdapter = new MyCourseListAdapter(getActivity(), listEntityList);
                ...
                mAdapter.setOnItemListener(new MyCourseListAdapter.OnItemClickListener() {
                    @Override
                    public void onClick(View v, int position, List<MyCourseListBean.BodyEntity.ListEntity> entity) {
                        if (entity != null) {
                            myCourseListEntity = entity.get(position);
                            Intent intent = new Intent(getActivity(), MajorClassDetailActivity.class);
                            intent.putExtra("myCourseListEntity", myCourseListEntity);
